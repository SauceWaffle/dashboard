function getGaugeValueColors(gauge) {
  for (s = 0; s < gauge.props.rangeSegments.length; s++ ) {
    if (  gauge.props.currentValue >= ( (gauge.props.maxValue < gauge.props.minValue) ? gauge.props.rangeSegments[s].end : gauge.props.rangeSegments[s].start )
       && gauge.props.currentValue <= ( (gauge.props.maxValue < gauge.props.minValue) ? gauge.props.rangeSegments[s].start : gauge.props.rangeSegments[s].end )
       ) {
          gauge.props.valueBoxColor = gauge.props.rangeSegments[s].color;
          gauge.props.valueBoxColorShaded = gauge.props.rangeSegments[s].rgba;
         }
  }

  if (!gauge.props.valueBoxColor) {
    if ( (gauge.props.maxValue < gauge.props.minValue && gauge.props.currentValue > gauge.props.maxValue)
      || (gauge.props.maxValue > gauge.props.minValue && gauge.props.currentValue < gauge.props.minValue)
    ){
      gauge.props.valueBoxColor = gauge.props.rangeSegments[0].color;
      gauge.props.valueBoxColorShaded = gauge.props.rangeSegments[0].rgba;
    }
    else {
      gauge.props.valueBoxColor = gauge.props.rangeSegments[2].color;
      gauge.props.valueBoxColorShaded = gauge.props.rangeSegments[2].rgba;
    }
  }
}


window.app.ports.loadSupplyGauges.subscribe(function(incoming) {
  for (g = 0; g < incoming.length; g++) {

    var gauge = new AquaGauge(incoming[g].id);

    gauge.props.rangeSegments[0].start = incoming[g].badStart;
    gauge.props.rangeSegments[0].end = incoming[g].badEnd;
    gauge.props.rangeSegments[1].start = incoming[g].okStart;
    gauge.props.rangeSegments[1].end = incoming[g].okEnd;
    gauge.props.rangeSegments[2].start = incoming[g].goodStart;
    gauge.props.rangeSegments[2].end = incoming[g].goodEnd;

    gauge.props.currentValue = incoming[g].value;
    gauge.props.dialSubTitle = incoming[g].title;
    gauge.props.minValue = incoming[g].min;
    gauge.props.maxValue = incoming[g].max;

    getGaugeValueColors(gauge);
    gauge.refresh();
...
function AquaGauge(target) {
    ...
    this.refresh = function () {
        ...
        var currAngle = ((this.props.endAngle - this.props.startAngle) * (this.props.gaugeValue - this.props.minValue)) / (this.props.maxValue - this.props.minValue) - this.props.endAngle;
    ...
    function drawDial(cvs, props) {
        ...
        // cvs.ctx.drawImage( props.QED_Logo,
        //                   cvs.cX - getRelVal(cvs, 35), cvs.cY - getRelVal(cvs, 90),
        //                   getRelVal(cvs, 70), getRelVal(cvs, 70) )
    ...
    function drawScale(cvs, props, drawGrad, drawValue) {
        ...
        var valInc = ( (props.maxValue < props.minValue) ? (props.minValue - props.maxValue) : (props.maxValue - props.minValue) ) / totalDivs;
        ...
        for (var i = 0; i < totalDivs+1; i++) {
            ...
            var currentVal = (props.maxValue < props.minValue) ? props.minValue - valInc * i : props.minValue + valInc * i;
            ...
            if (i % props.noOfSubDivisions == 0) {
                ...
                x = getX(cvs.cX, textRadius, currentAngle + (.2 * i));
                y = getY(cvs.cY, textRadius, currentAngle + (.2 * i));
    ...
    function drawCurrentValue(cvs, currValue, props) {
        ...
        cvs.ctx.fillStyle = props.valueBoxColorShaded;
        cvs.ctx.strokeStyle = props.valueBoxColor;
    ...
    function drawGlosiness(cvs, props) {
        ...
        grd.addColorStop(1, "rgba(255, 255, 255, 0.17)");
        cvs.ctx.fillStyle = grd;
        cvs.ctx.beginPath();
    ...
    function validateProps(props) {
      ...
      if (props.currentValue >= 1000)
        props.currentValue = Math.rount(props.currentValue);
      else if ( props.currentValue >= 100)
        props.currentValue = props.currentValue.toFixed(1);
      else
        props.currentValue = props.currentValue.toFixed(2);


      if (props.currentValue % 1 == 0)
        props.currentValue = Math.round(props.currentValue);


      props.gaugeValue = props.currentValue;

      if (props.maxValue <= props.minValue)
        //props.maxValue = props.minValue + 10;
        props.gaugeValue = props.currentValue < props.minValue ? (props.currentValue < props.maxValue ? props.maxValue : props.currentValue) : props.minValue;
      else
        props.gaugeValue = props.currentValue > props.minValue ? (props.currentValue > props.maxValue ? props.maxValue : props.currentValue) : props.minValue;
...
function GaugeParams(cvs) {
    ...
    this.dialColor = "rgba(223, 223, 223, 0)";
    this.dialGradientColor = "rgba(209, 211, 227, 0.9)";
    this.dialValueTextFont = getRelVal(cvs, 27) + "px Arial Black";
    this.dialValueTextColor = "#757575";
    this.dialTitleTextFont = "bold " + getRelVal(cvs, 18) + "px Michroma";
    ...
    //Scale Ranges
    this.rangeSegments = [{ type: "bad", start: 0, end: 40, color: "#FF6D63", rgba: "rgba(255, 109, 99, 0.5)" },
                          { type: "ok", start: 40, end: 75, color: "#FFEA75", rgba: "rgba(255, 234, 117, 0.5)" },
                          { type: "good", start: 75, end: 100, color: "#A7FFA5", rgba: "rgba(167, 255, 165, 0.5)" }];
    //this.rangeSegments = [{ start: 0, end: 100, color: "#666" }];
    ...
    //Pointer
    this.pointerColor = "rgba(75, 75, 75, 0.8)";
    this.pointerGradientColor = "rgba(130, 130, 130, 0.8)";
    this.shadowColor = "#454545";

    // Logo
    this.QED_Logo = document.getElementById('qedlogo');
